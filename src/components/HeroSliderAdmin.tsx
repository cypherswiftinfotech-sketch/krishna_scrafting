...
interface Props {
  apiPath: string;       // e.g. "/api/blogs-hero"
  title: string;         // e.g. "Blogs Hero Slider"
  description?: string;
}

export default function HeroSliderAdmin({ apiPath, title, description }: Props) {
    ...
    <div className="p-6 rounded-2xl shadow bg-white" style={{ border: "1px solid var(--cream-white-border)" }}>
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {description || "Upload images that will auto-scroll continuously on the page hero section."}
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="py-16 text-center">
          ...
          <p className="text-sm text-gray-300 mt-1">Upload images to display them in the hero slider.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {images.map((img, idx) => (
            <div
              ...
              style={{ borderColor: "var(--cream-white-border)" }}
            >
              <Image src={img.mediaUrl} alt={`Hero image ${idx + 1}`} fill className="object-cover" />
      ...
      {images.length > 0 && (
        <p className="text-sm text-gray-400 mt-4 text-center">
          {images.length} image{images.length !== 1 ? "s" : ""} — hover to delete. Images auto-scroll on the page.
        </p>
      )}
    </div>
  );
